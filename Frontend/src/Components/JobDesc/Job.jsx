    ...
    return <div data-aos="zoom-out" className="w-2/3 bs-mx:w-full">
        <div className="flex justify-between items-center flex-wrap">
            <div className="flex items-center gap-2">
                <div className="p-3 bg-[#212529] rounded-xl shrink-0 flex ">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="text-lg text-[#ced4da] flex flex-wrap xs-mx:text-base"><span>{props.company} &bull; </span><span> {timeAgo(props.postTime||"")} &bull; </span> <span>{props.applicants?props.applicants.length:0} Applicants </span></div>
                </div>
        ...
        <div className="flex justify-between gap-4 sm-mx:flex-wrap">
            {
                card.map((item, index) => <div key={index} className="flex flex-col text-sm gap-1 items-center ">
                    ...
                    <div className="text-[#ced4da] xs-mx:text-sm">{item.name}</div>
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-2">
                {
                    props.skillsRequired?.map((skill, index) => <ActionIcon key={index} className="h-fit! w-fit! font-medium text-sm! xs-mx:!text-xs" variant="light" color="brightSun.4" p="xs" radius="xl">{skill}</ActionIcon>)
                }
            </div>
        </div>
        <Divider size="xs" my="xl" />
        <div className="[&>h4]:text-xl [&_*]:text-[#ced4da] [&_li]:marker:text-[#adb5bd] [&_li]:mb-1 [&>h4]:text-[#dee2e6] [&>h4]:font-semibold [&>h4]:my-5 [&_p]:text-justify [&_p]:text-sm [&_li]:text-sm" dangerouslySetInnerHTML={{ __html: cleanHTML }}>
        </div>
        <Divider size="xs" my="xl" />
        <div>
