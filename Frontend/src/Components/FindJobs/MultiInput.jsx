        ...
        .filter((item) => item.toLowerCase().includes(search.trim().toLowerCase())).map((item, index) => (
            ...
            >
                <Group gap="sm">
                    ...
                    />
                    <span className='text-[#ced4da]'>{item}</span>
                </Group>
            </Combobox.Option>
        ));
        ...
        <Combobox  store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
            <Combobox.DropdownTarget>
                <PillsInput variant='unstyled' size="sm" pointer onClick={() => combobox.toggleDropdown()}
                    leftSection={<div className="bg-[#1a1a1a] rounded-full mr-2 text-[#4682B4] p-1"><props.icon size={20} /> </div>}
                ...
                >
                    ...
                    <Pill.Group>
                        {value.length > 0 ? (
                            ...
                            <Input.Placeholder className='text-[#ced4da]!'>{props.title}</Input.Placeholder>
                        )}
