    ...
    return <div className="w-1/5">
        ...
        <div>
            <Tabs variant="pills" autoContrast value={activeTab} onChange={setActiveTab}>
                <Tabs.List className="[&_button[aria-selected='false']]:bg-[#1a1a1a] font-medium">
        ...
        <div className="flex flex-col flex-wrap mt-5 gap-5">
            {
              props.jobList.filter((job)=>job?.jobStatus===activeTab).sort((a, b) => new Date(b.postTime).getTime() - new Date(a.postTime).getTime()).map((item, index)=> <PostedJobCard key={index} {...item}/>)
